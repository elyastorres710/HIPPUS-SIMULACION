...
def calcular_pual(señal: np.ndarray, fs: float) -> float:
    ...
    mask_gufoni = (freqs >= 0.2) & (freqs <= 2.5)
    pual_absoluto = np.sum(fft_vals[mask_gufoni])
...
def calcular_pual_ratio(señal: np.ndarray, fs: float) -> float:
    ...
    mask_casani = (freqs >= 0.04) & (freqs <= 2.0)
...
def calcular_frecuencia_dominante(señal: np.ndarray, fs: float) -> float:
    ...
    mask = (frecuencias >= 0.2) & (frecuencias <= 2.5)
    if not np.any(mask): return 0.0
    
    indice_max = np.argmax(fft_vals[mask])
    return float(frecuencias[mask][indice_max])

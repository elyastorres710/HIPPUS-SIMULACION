...
seleccionado automáticamente por el ranking de la iteración 1, con el
objetivo de explicar qué biomarcadores del hippus pupilar impulsan la
clasificación de Migraña Vestibular vs. Control.

Entradas:
    - data/processed/analisis_resultados.csv       (biomarcadores por sujeto)
    - scripts/iteracion_1/metricas_completas.csv   (ranking de combinaciones)

Salidas (data/test/):
    - xai_it1_importancia_global.png   : importancia media |SHAP| por biomarcador
    - xai_it1_summary_beeswarm.png     : distribución de impacto por sujeto/característica
    - xai_it1_caso_migrana.png         : explicación individual — caso Migraña Vestibular
    - xai_it1_caso_control.png         : explicación individual — caso Control
    - xai_it1_dependencia.png          : dependencia SHAP vs. valor real por biomarcador
    - xai_it1_reporte.txt              : resumen cuantitativo de importancias SHAP

Ejecución:
    python scripts/iteracion_1/07_xai_shap.py
"""
...
# ---------------------------------------------------------------------------
# Constantes de configuración (alineadas con scripts 03 y 06)
# ---------------------------------------------------------------------------
PATH_DATOS: str = 'data/processed/analisis_resultados.csv'
PATH_RANKING: str = 'scripts/iteracion_1/metricas_completas.csv'
...
def cargar_datos_y_biomarcadores(
    ...
    combinación de variables seleccionada automáticamente por el script 03.

    Args:
        path_datos: Ruta al CSV con biomarcadores extraídos por sujeto.
        path_ranking: Ruta al CSV de ranking de combinaciones (metricas_completas.csv).
    ...
    biomarcadores: list = ast.literal_eval(df_ranking.iloc[0]['Variables'])
...
def entrenar_modelo_rf(
    ...
    """
    Entrena el Random Forest con la configuración estándar de la iteración 1.
...
def guardar_importancia_global(
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(path_salida, 'xai_it1_importancia_global.png'),
        dpi=DPI_FIGURA, bbox_inches='tight'
    )
    plt.close(fig)
    print("  [OK] xai_it1_importancia_global.png")
...
def guardar_summary_beeswarm(
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(path_salida, 'xai_it1_summary_beeswarm.png'),
        dpi=DPI_FIGURA, bbox_inches='tight'
    )
    plt.close(fig)
    print("  [OK] xai_it1_summary_beeswarm.png")
...
def guardar_graficos_dependencia(
    ...
    plt.tight_layout()
    fig.savefig(
        os.path.join(path_salida, 'xai_it1_dependencia.png'),
        dpi=DPI_FIGURA, bbox_inches='tight'
    )
    plt.close(fig)
    print("  [OK] xai_it1_dependencia.png")
...
def guardar_reporte_texto(
    ...
    lineas = [
        "=== REPORTE XAI — SHAP sobre Random Forest (Iteración 1) ===",
    ...
    ruta_txt = os.path.join(path_salida, 'xai_it1_reporte.txt')
    with open(ruta_txt, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lineas) + '\n')

    print("  [OK] xai_it1_reporte.txt")
    print('\n'.join(lineas))
...
def main() -> None:
    """
    Ejecuta el pipeline completo de explicabilidad SHAP para la iteración 1.
    ...
    print("\n=== Script 07 — XAI SHAP | Iteración 1 ===\n")
    ...
    guardar_caso_individual(
        ...
        etiqueta_clase=1,
        nombre_archivo='xai_it1_caso_migrana',
    ...
    guardar_caso_individual(
        shap_vals, valor_esperado, X_test, y_test,
        etiqueta_clase=0,
        nombre_archivo='xai_it1_caso_control',
